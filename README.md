...
- **In-Discord gamified pre-show** — the `/watch` menu is **private (ephemeral)**,
  ...
  watches: a sequence of **animated Canvas GIFs** — **Box Office** (curtains part
  to reveal the owner's poster/title from Cloudflare) → **🎟️ Ticket** (prints out
  with your avatar + seat) → **🍿 Concession stand** (popcorn pops, random snack) →
  **🪑 Seat** (you drop in, lights dim) → **🎬 Enter Theater**, then it **vanishes** as
  the user drops into the movie. Only the person who started it can
  press the buttons; anyone else who taps is nudged to run `/watch` themselves.
